    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                if insect.can_contain(self.ant) and not self.ant.can_contain(insect):
                    insect.contain_ant(self.ant)
                    self.ant = insect
                elif not insect.can_contain(self.ant) and self.ant.can_contain(insect):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 5
        current_search = self.place
        current_range = 0
        while current_search is not hive:
            if current_search.bees and current_range >= self.min_range and current_range <= self.max_range:
                return random_or_none(current_search.bees)
            current_range += 1
            current_search = current_search.entrance        
        # END Problem 5
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while back.exit:
            back = back.exit
            if back.ant:
                if back.ant not in self.doubled_ants:
                    back.ant.damage *= 2
                    self.doubled_ants += [back.ant]
                if back.ant.container and back.ant.ant and back.ant.ant not in self.doubled_ants:
                    back.ant.ant.damage *= 2
                    self.doubled_ants += [back.ant.ant]
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    def combine(func, obj):
        return lambda *args: func(obj, *args)

    new_action = combine(effect(bee.action), bee)
