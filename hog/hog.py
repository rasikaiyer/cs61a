...
def free_bacon(score):
	...
	# BEGIN PROBLEM 2
	total = 0
	current_max = score % 10
	score = score // 10
	while score:
		digit = score % 10
		score = score // 10
		current_max = max(digit, current_max)
...
def announce_highest(who, previous_high=0, previous_score=0):
	...
	def say(score0, score1):
		...
		if new_high:
			...
			# else:
			# 	print("Player", who, "gets", new_high , "points; not enough for a new high")
		return announce_highest(who, max(new_high, previous_high), new_score)
	return say
	# END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
	...
	# BEGIN PROBLEM 9
	k, dice_max = 10, 10
	points_max = make_averaged(roll_dice, num_samples)(k, dice)
	k -= 1
	while k > 0:
		if (points_max < make_averaged(roll_dice, num_samples)(k, dice)):
			points_max = make_averaged(roll_dice, num_samples)(k, dice)
			dice_max = k
		k -= 1
	return dice_max
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
	...
	if (((opponent_score > score) and (opponent_score % new_score == 0)) or (free_bacon(opponent_score) >= margin)):
		return 0
	else:
		return num_rolls
	# END PROBLEM 11
