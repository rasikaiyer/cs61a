...
CREATE TABLE calories AS
  SELECT count(a.calories + b.calories) FROM main_course AS a, pies AS b WHERE a.calories + b.calories < 2500;

CREATE TABLE healthiest_meats AS
 SELECT meat, min(a.calories + b.calories) as calories FROM main_course as a, pies as b GROUP BY meat HAVING max(a.calories + b.calories) < 3000;

CREATE TABLE average_prices AS
  SELECT category, avg(msrp) FROM products GROUP BY category;

CREATE TABLE lowest_prices AS
  SELECT store, name, price FROM inventory, products WHERE name = item GROUP BY name HAVING min(price);

CREATE TABLE shopping_list AS
  SELECT a.name, store FROM products AS b, lowest_prices AS a WHERE a.name = b.name GROUP BY category HAVING min(MSRP/rating);
