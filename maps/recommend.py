...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    locations = [restaurant_location(c) for c in cluster]
    return [mean([l[0] for l in locations]), mean([l[1] for l in locations])]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(c) for c in cluster if c != []]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for f in feature_fns:
        x , y = find_predictor(user, reviewed, f)
        r_list[x] = y
    return max(r_list, key = r_list.get)
    # END Question 8
