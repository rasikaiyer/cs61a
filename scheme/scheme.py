...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(self, operands.map(lambda arg: scheme_eval(arg, env)), env)
        # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    test_value = scheme_eval(expressions.first, env)
    if expressions.second == nil and scheme_truep(test_value):
        return test_value
    elif not scheme_truep(test_value):
        return False
